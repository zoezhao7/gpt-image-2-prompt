...
# -*- coding: utf-8 -*-
"""
Build a fully self-contained search-engine.html with ALL data inline.
No external data.js needed → works under file:// protocol.
"""
...
# ---------- Build safe JS: put data in <script type="application/json"> then read it ----------
# Approach: embed JSON in a script tag with type=application/json, read via DOM

cases_js = json.dumps(cases, ensure_ascii=False, separators=(',', ':'))
cats_js  = json.dumps(categories, ensure_ascii=False)
styles_js = json.dumps(styles_list, ensure_ascii=False)
scenes_js = json.dumps(scenes_list, ensure_ascii=False)
...
tpl_js = json.dumps(TEMPLATES, ensure_ascii=False, separators=(',', ':'))
...
<!-- Data embedded as application/json (not executed by browser) -->
  ...
  div.innerHTML =
    ...
    '<div class="card-body">' +
      ...
      '<div class="card-actions">' +
        ...
        '<button class="btn-view" onclick="event.stopPropagation();openModal(\'' + id + '\',\'' + currentMode + '\')">查看</button>' +
      '</div>' +
    '</div>';

  div.addEventListener('click', function(){ openModal(id, currentMode); });
...
print(f'   Data: embedded inline (works under file://)')
